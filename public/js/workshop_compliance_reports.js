...
function serviceCompliance(bus){const currentRaw=bus.currentOdometer??bus.odometer??bus.odometerKm,dueRaw=bus.nextServiceOdometer??bus.nextServiceKm,current=currentRaw==null||currentRaw===""?NaN:Number(currentRaw),dueKm=dueRaw==null||dueRaw===""?NaN:Number(dueRaw),date=bus.nextServiceDate||"";const days=daysFromToday(date),remaining=dueKm-current,hasKm=Number.isFinite(current)&&Number.isFinite(dueKm);if((hasKm&&remaining<=0)||(days!=null&&days<0))return{level:"Critical",label:"OVERDUE"};if((hasKm&&remaining<=1000)||(days!=null&&days<=14))return{level:"Attention",label:"DUE SOON"};if(hasKm&&remaining<=4000)return{level:"Attention",label:"PLAN SERVICE"};if((Number.isFinite(dueKm)&&dueKm>0)||date)return{level:"Compliant",label:"ON TRACK"};return{level:"Attention",label:"NOT SET"};}
function airconCompliance(bus){const fitted=norm(bus.airConditioned);if(["no","false","not fitted","n/a"].includes(fitted))return{level:"Compliant",label:"N/A",date:""};const date=bus.nextAirConditioningServiceDate||"",days=daysFromToday(date);if(days==null)return{level:"Attention",label:"NOT SET",date};if(days<0)return{level:"Critical",label:"OVERDUE",date};if(days<=30)return{level:"Attention",label:days===0?"DUE TODAY":"DUE SOON",date};return{level:"Compliant",label:"ON TRACK",date};}
...
function fleetComplianceRow(bus){const rego=regoCompliance(bus),service=serviceCompliance(bus),aircon=airconCompliance(bus),safety=dueState(bus),openDefects=fleetDefects(bus),openJobs=fleetJobs(bus);const critical=rego.level==="Critical"||service.level==="Critical"||aircon.level==="Critical"||safety.kind==="bad"||openDefects.some(defect=>defect.safeToDrive==="No")||/out of service/i.test(bus.status||"");const attention=rego.level==="Attention"||service.level==="Attention"||aircon.level==="Attention"||["warn","missing"].includes(safety.kind)||openDefects.length||openJobs.length||/workshop|restricted/i.test(bus.status||"");return{bus,rego,service,aircon,safety,openDefects,openJobs,overall:critical?"Critical":attention?"Attention":"Compliant"};}
...
function printFleet(){printTableReport("Fleet Compliance Summary",["Fleet","Rego","Depot","Registration","Service","Annual A/C","90-Day","Defects","Jobs","Vehicle Status","Overall"],filteredFleet().map(row=>[fleetNo(row.bus),row.bus.rego||"—",row.bus.depot||"—",row.rego.label,row.service.label,row.aircon.label,row.safety.label,String(row.openDefects.length),String(row.openJobs.length),row.bus.status||"Active",row.overall]));}
