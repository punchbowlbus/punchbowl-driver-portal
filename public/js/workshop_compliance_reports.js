...
function defectDone(defect){return ["completed","closed"].includes(norm(defect.status));}
...
function filteredDefects(){const term=norm($("driverDefectReportSearch")?.value),status=$("driverDefectReportStatus")?.value||"",category=$("driverDefectReportCategory")?.value||"",safety=$("driverDefectReportSafety")?.value||"",from=$("driverDefectReportFrom")?.value||"",to=$("driverDefectReportTo")?.value||"";return defects.filter(defect=>{const date=defectDate(defect),haystack=norm([defect.reportNumber,defect.fleetNumber,defect.rego,defect.reportedByName,defect.category,defect.description,defect.status].join(" "));return(!term||haystack.includes(term))&&(!status||(defect.status||"New")===status)&&(!category||defect.category===category)&&(!safety||defect.safeToDrive===safety)&&(!from||(date&&date>=from))&&(!to||(date&&date<=to));});}
