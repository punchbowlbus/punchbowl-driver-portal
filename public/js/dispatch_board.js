...
els.contentArea.innerHTML = `
  <section id="dispatchBoardShell" class="dispatch-pro-shell" aria-label="Dispatch Board">
    ...
    <div class="dispatch-pro-toolbar">
      ...
      <label class="dispatch-driver-picker"><span>Select drivers</span>
        <div id="dispatchDriverMultiSelect" class="dispatch-driver-multi-select">
          <div id="selectedDriverChips" class="dispatch-driver-chips"></div>
          <input id="driverSearchFilter" type="search" placeholder="Search and add drivers" autocomplete="off" aria-expanded="false" aria-controls="driverSearchSuggestions" />
          <div id="driverSearchSuggestions" class="dispatch-driver-suggestions" hidden></div>
        </div>
      </label>
      <button id="clearDriverFilterBtn" class="dispatch-quiet-btn" type="button" hidden>Clear all</button>
    </div>
...
const driverMultiSelectEl = document.getElementById("dispatchDriverMultiSelect");
const selectedDriverChipsEl = document.getElementById("selectedDriverChips");
const driverSearchSuggestionsEl = document.getElementById("driverSearchSuggestions");
...
const selectedDriverEmpNos = new Set();
  ...
  function getActiveDrivers() {
    let drivers = employeesCache.filter((e) => {
      ...
      if (selectedDriverEmpNos.size && !selectedDriverEmpNos.has(empNo)) return false;
  ...
  function getDriverSelectorName(driver) {
    return String(
      driver.displayName ||
  ...
  function getDriverSelectorCandidates(query = "") {
    const search = String(query || "").trim().toLowerCase();
    return employeesCache
      .filter((driver) => {
        const role = String(driver.role || "").toLowerCase().trim();
        const status = String(driver.status || "").toLowerCase().trim();
        const empNo = String(driver.employeeNumber || "");
        if (role !== "driver" || status === "inactive" || selectedDriverEmpNos.has(empNo)) return false;
        if (!search) return false;
        const haystack = [driver.displayName, driver.firstName, driver.lastName, empNo]
          .filter(Boolean).join(" ").toLowerCase();
        return haystack.includes(search);
      })
      .sort((a, b) => getDriverSelectorName(a).localeCompare(getDriverSelectorName(b)))
      .slice(0, 10);
  }

  function renderDriverMultiSelect() {
    if (!selectedDriverChipsEl || !driverSearchSuggestionsEl) return;

    const selectedDrivers = [...selectedDriverEmpNos]
      .map((empNo) => employeesCache.find((driver) => String(driver.employeeNumber || "") === empNo))
      .filter(Boolean);

    selectedDriverChipsEl.innerHTML = selectedDrivers.map((driver) => {
      const empNo = String(driver.employeeNumber || "");
      const name = getDriverSelectorName(driver);
      return `<span class="dispatch-driver-chip"><span>${escapeHtml(name)}</span><small>${escapeHtml(empNo)}</small><button type="button" data-remove-selected-driver="${escapeHtml(empNo)}" aria-label="Remove ${escapeHtml(name)}">×</button></span>`;
    }).join("");

    selectedDriverChipsEl.querySelectorAll("[data-remove-selected-driver]").forEach((button) => {
      button.onclick = () => {
        selectedDriverEmpNos.delete(String(button.dataset.removeSelectedDriver || ""));
        renderDriverMultiSelect();
        renderDrivers();
      };
    });

    const candidates = getDriverSelectorCandidates(driverSearchFilterEl?.value || "");
    driverSearchSuggestionsEl.innerHTML = candidates.map((driver) => {
      const empNo = String(driver.employeeNumber || "");
      const type = String(driver.employeeType || driver.employmentType || "Driver");
      return `<button type="button" data-add-selected-driver="${escapeHtml(empNo)}"><span><strong>${escapeHtml(getDriverSelectorName(driver))}</strong><small>${escapeHtml(type)}</small></span><b>${escapeHtml(empNo)}</b></button>`;
    }).join("");
    driverSearchSuggestionsEl.hidden = !candidates.length;
    driverSearchFilterEl?.setAttribute("aria-expanded", String(candidates.length > 0));
    clearDriverFilterBtn.hidden = !selectedDriverEmpNos.size && !driverSearchFilterEl?.value;

    driverSearchSuggestionsEl.querySelectorAll("[data-add-selected-driver]").forEach((button) => {
      button.onclick = () => {
        selectedDriverEmpNos.add(String(button.dataset.addSelectedDriver || ""));
        driverViewFilterEl.value = "all";
        driverSearchFilterEl.value = "";
        renderDriverMultiSelect();
        renderDrivers();
        driverSearchFilterEl.focus();
      };
    });
  }
      ...
      <section class="dispatch-assigned-duty-group">
        ...
        <div class="dispatch-assigned-duty-jobs">
          ${jobs.map((block, blockIndex) => {
            ...
            return `<article data-job-detail="${escapeHtml(block.id || "")}" class="dispatch-job-detail-card ${isSelectedJob ? "selected" : ""}" style="--job-group-color:${colors.bg};--job-group-text:${colors.text}" title="${escapeHtml(hoverDetails)}">
              ...
              ${notes ? `<div class="dispatch-job-notes"><strong>Notes:</strong> ${escapeHtml(notes)}</div>` : ""}
...
function getCompactGroupName(value) {
  return String(value || "No Group")
    .replace(/\bPUBLIC SCHOOL\b/gi, "PS")
    .replace(/\bPRIMARY SCHOOL\b/gi, "PS")
    .replace(/\bHIGH SCHOOL\b/gi, "HS")
    .replace(/\bSECONDARY COLLEGE\b/gi, "SC")
    .replace(/\bCOLLEGE\b/gi, "COL")
    .replace(/\s+/g, " ")
    .trim();
}
...
function renderUnassignedTimeline(blocks, selectedDate) {
  ...
  const laneHeight = 42;
  const groupGap = 8;
  const minimumReadableBlockWidth = 118;
  const minimumReadableMinutes = (minimumReadableBlockWidth / slotWidth) * SLOT_MINUTES;
    ...
    .map((group) => {
      ...
      const placed = group.entries
        ...
        .map((entry) => {
          ...
          const visualAxisEnd = Math.max(axisEnd, axisStart + minimumReadableMinutes);
          let lane = laneEnds.findIndex((lastEnd) => lastEnd <= axisStart);
          if (lane < 0) lane = laneEnds.length;
          laneEnds[lane] = visualAxisEnd;
  ...
  unassignedJobsPanelBodyEl.innerHTML = `<div class="dispatch-unassigned-canvas" style="width:${width}px;height:${canvasHeight}px">
    ...
    ${groupRows.map((group) => {
      ...
      return `<div class="dispatch-unassigned-group-band" style="top:${group.top}px;height:${group.height}px;--job-group-border:${colors.border}"></div>${group.placed.map(({ block, code, set, lane, start, end }) => {
        ...
        const blockWidth = Math.max(minimumReadableBlockWidth, ((end - start) / SLOT_MINUTES) * slotWidth);
        const isSelected = String(selectedBlockId) === String(block.id || "");
        const runCode = getRouteRunCode(block);
        const displayCode = runCode || code;
        const busLabel = getBusLabel(block);
        const compactGroupName = getCompactGroupName(set.groupName);
        ...
        const title = `${busLabel || displayCode} · ${set.groupName} · ${minToTimeStr(start)}-${minToTimeStr(end)}${block.notes ? ` · Notes: ${block.notes}` : ""}${route !== " → " ? ` · ${route}` : ""}`;
        return `<button type="button" draggable="true" class="dispatch-unassigned-block ${isSelected ? "selected" : ""}" data-block-id="${escapeHtml(block.id || "")}" data-code="${escapeHtml(displayCode)}" data-start="${start}" data-end="${end}" style="--job-group-bg:${colors.bg};--job-group-border:${colors.border};--job-group-text:${colors.text};left:${left}px;top:${group.top + lane * laneHeight + 4}px;width:${blockWidth}px" title="${escapeHtml(title)}"><span class="dispatch-unassigned-card-top"><strong>${escapeHtml(busLabel || displayCode)}</strong><span>${escapeHtml(minToTimeStr(start))}–${escapeHtml(minToTimeStr(end))}</span></span><span class="dispatch-unassigned-route">${escapeHtml(compactGroupName)}</span></button>`;
      }).join("")}`;
    }).join("")}
  </div>`;
...
driverSearchFilterEl.onkeydown = (event) => {
  if (event.key === "Enter") {
    const firstSuggestion = driverSearchSuggestionsEl.querySelector("[data-add-selected-driver]");
    if (firstSuggestion) {
      event.preventDefault();
...
driverSearchFilterEl.onfocus = renderDriverMultiSelect;
clearDriverFilterBtn.onclick = () => {
  selectedDriverEmpNos.clear();
  driverSearchFilterEl.value = "";
  renderDriverMultiSelect();
  renderDrivers();
};
document.addEventListener("click", (event) => {
  if (!driverMultiSelectEl?.contains(event.target)) {
    driverSearchSuggestionsEl.hidden = true;
    driverSearchFilterEl.setAttribute("aria-expanded", "false");
  }
});
