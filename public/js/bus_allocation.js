...
function dutyLocationHtml(duty) {
  const type = norm(duty.dutyType);
  const start = clean(duty.startLocation);
  const end = clean(duty.endLocation);
  let startLabel = "Start location";
  let endLabel = "End location";

  if (type.includes("rail")) {
    startLabel = "Start station";
    endLabel = "End station";
  } else if (type.includes("charter")) {
    startLabel = "Start depot";
    endLabel = "End depot";
  }

  return `<small class="ba-location"><span><b>${startLabel}:</b> ${esc(start || "Not set")}</span><span><b>${endLabel}:</b> ${esc(end || "Not set")}</span></small>`;
}
...
function rowHtml(duty) {
  ...
  return `<tr class="ba-row ba-${status.cls}" data-duty-id="${esc(duty.id)}">
    ...
    ${relief ? `
      ...
      <td><span class="ba-type ${vehicleRequirement(duty) === "EV" ? "ev" : ""}">${esc(vehicleRequirement(duty))}</span>${dutyLocationHtml(duty)}</td>`}
...
function printLocation(duty) {
  if (isRelief(duty)) return clean(duty.changeoverLocation);
  const start = clean(duty.startLocation);
  const end = clean(duty.endLocation);
...
function printVehicle(duty) {
  const number = isRelief(duty)
    ? clean(duty.supportVehicle || duty.reliefBusNumber)
    : clean(duty.actualBus || duty.assignedBus);
  if (!number) return "";
  return vehicleRequirement(duty) === "EV" ? `${number} · EV` : number;
}

function printRowHtml(duty) {
  const hasStart = clean(duty.startMin) !== "";
  const hasEnd = clean(duty.endMin) !== "";
  const depart = clean(duty.yardDepartureMin) !== ""
    ? minutes(duty.yardDepartureMin)
    : (hasStart ? minutes(duty.startMin) + 5 : null);
  return `<tr>
    <td><strong>${esc(duty.dutyNumber)}</strong><small>${esc(duty.dutyType)}</small></td>
    <td><strong>${esc(duty.driverName)}</strong><small>${esc(duty.driverEmployeeNumber)}</small></td>
    <td>${hasStart ? formatTime(duty.startMin) : ""}</td>
    <td>${depart == null ? "" : formatTime(depart)}</td>
    <td class="ba-print-bus">${esc(printVehicle(duty))}</td>
    <td>${esc(printLocation(duty))}</td>
    <td>${hasEnd ? formatTime(endMinute(duty)) : ""}</td>
  </tr>`;
}
...
function renderTable() {
  ...
  const printBody = document.getElementById("baPrintRows");
  if (printBody) {
    printBody.innerHTML = order.filter((section) => grouped.has(section)).map((section) => `<tr class="ba-section"><td colspan="7">${section}</td></tr>${grouped.get(section).map(printRowHtml).join("")}`).join("")
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    @media print{@page{size:210mm 297mm;margin:8mm}html,body{width:194mm!important;min-width:0!important;margin:0!important;background:#fff!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}.app-shell,.main-content,#contentArea{display:block!important;width:100%!important;max-width:none!important;margin:0!important;padding:0!important}.sidebar,.topbar,.ba-head,.ba-controls,.ba-metrics,.ba-alerts,.ba-screen,.ba-screen-table{display:none!important}.ba-page,.ba-layout{display:block!important;width:100%!important;margin:0!important;padding:0!important}.ba-panel{width:100%!important;overflow:visible!important;border:0!important;border-radius:0!important;box-shadow:none!important}.ba-panel-head{display:none!important}.ba-print-title{display:block!important;width:100%;box-sizing:border-box;text-align:center;border:2px solid #000;margin:0 0 7px;break-after:avoid}.ba-print-title h1{margin:0;padding:7px;background:#000!important;color:#fff!important;font-size:20px;font-weight:900;line-height:1.15}.ba-print-title h2{margin:0;padding:6px;font-size:16px;font-weight:900;line-height:1.1}.ba-print-only{display:table!important;width:100%!important;max-width:100%!important;table-layout:fixed;border-collapse:collapse;font-size:12px}.ba-print-only th,.ba-print-only td{border:1px solid #000;padding:7px;vertical-align:middle;text-align:left;overflow-wrap:anywhere}.ba-print-only th{font-size:11px;font-weight:900;text-transform:uppercase}.ba-print-only td>small{display:block;margin-top:2px;font-size:9px}.ba-print-only .ba-print-bus{font-size:20px;font-weight:900}.ba-print-only .ba-section td{background:#000!important;color:#fff!important;padding:5px 7px!important;font-size:11px;font-weight:900}.ba-print-only thead{display:table-header-group}.ba-print-only tr{break-inside:avoid;page-break-inside:avoid}.ba-toast{display:none!important}}
  `;
...
export function renderBusAllocationPage() {
  ...
  els.contentArea.innerHTML = `<div class="ba-page">
    ...
    <div class="ba-layout"><section class="ba-panel"><div class="ba-print-title"><h1>PUNCHBOWL BUS COMPANY — DAILY DEPARTURE SHEET</h1><h2 id="baPrintDate"></h2></div><div class="ba-panel-head"><div><h2>Departure and Changeover Plan</h2><small>Workshop and Out-of-Service buses are excluded automatically · ${TURNAROUND_MINUTES}-minute reuse buffer</small></div></div><div class="ba-table-wrap ba-screen-table"><table class="ba-table"><thead><tr><th>Duty</th><th>Driver</th><th>Start</th><th>Depart</th><th>Movement</th><th>Planned bus / Support car</th><th>Actual bus / Bus on road</th><th>Vehicle / Location</th><th>Return</th><th>Status</th><th class="ba-screen">Confirm</th></tr></thead><tbody id="baRows"></tbody></table></div><table class="ba-print-only"><colgroup><col style="width:12%"><col style="width:18%"><col style="width:8%"><col style="width:8%"><col style="width:17%"><col style="width:27%"><col style="width:10%"></colgroup><thead><tr><th>Duty</th><th>Driver</th><th>Start</th><th>Depart</th><th>Allocated Bus</th><th>Location</th><th>Return</th></tr></thead><tbody id="baPrintRows"></tbody></table></section><aside class="ba-panel ba-alerts"><div class="ba-panel-head"><h2>Planning alerts</h2></div><div id="baAlerts"></div></aside></div>
  </div>`;
