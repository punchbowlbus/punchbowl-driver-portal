 ...
 * 3. sendGeneralPushNotification — callable: send an admin broadcast
 */
...
/* =========================================================
   General push notification — Callable from Settings
========================================================= */
exports.sendGeneralPushNotification = onCall({
  ...
  for (let start = 0; start < tokens.length; start += 500) {
    ...
    response.responses.forEach((result) => {
      if (!result.success) {
        failureCodes.push(result.error?.code || "unknown");
      }
    });
  }
